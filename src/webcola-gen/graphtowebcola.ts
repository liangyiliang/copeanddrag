...
export class WebColaLayout {
  ...
  constructor(
    ...
    fig_height: number = 800,
    fig_width: number = 800
  ) {
    ...
    this.colaConstraints = instanceLayout.constraints.map((constraint) =>
      this.toColaConstraint(constraint)
    );
  ...
  private toColaConstraint(constraint: LayoutConstraint): any {
    ...
    if (isLeftConstraint(constraint)) {
      ...
      return this.leftConstraint(
        this.getNodeIndex(constraint.left.id),
        this.getNodeIndex(constraint.right.id),
        distance
      );
    }
    ...
    if (isTopConstraint(constraint)) {
      ...
      node2.fixed = 0;
      let distance =
        constraint.minDistance + node1.height / 2 + node2.height / 2;

      return this.topConstraint(
        this.getNodeIndex(constraint.top.id),
        this.getNodeIndex(constraint.bottom.id),
        distance
      );
    }
  ...
  private determineGroups(
    groups: LayoutGroup[]
  ): { leaves: number[]; padding: number; name: string; groups: number[] }[] {
  ...
  private determineGroupsAndSubgroups(
    ...
    Object.entries(groupDefinitions).forEach(([key1, value1]) => {
      Object.entries(groupDefinitions).forEach(([key2, value2]) => {
        ...
        const shouldAddSubgroup =
          avoidContainmentCycle && this.isSubGroup(value2, value1);
    ...
    const colaGroupsBeforeSubgrouping = Object.entries(groupDefinitions).map(
      ([key, value]) => {
        ...
        let padding = name.startsWith(disconnectedNodeMarker)
    ...
    const colaGroups = Object.entries(colaGroupsBeforeSubgrouping).map(
      ([key, value]) => {
        ...
        let groups = subgroups[name].map((subgroupName) => {
          ...
          let subgroupIndex = colaGroupsBeforeSubgrouping.findIndex(
            (group) => group.name === subgroupName
          );
